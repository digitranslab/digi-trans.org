  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <main>
        ...
        <section className="relative pt-36 pb-24 overflow-hidden">
          ...
          <div className="container mx-auto px-4 relative z-10">
            <AnimatedWrapper animation="fade-up-slow" className="max-w-4xl mx-auto text-center">
              ...
              <TypewriterGradientText text={useCase.hero.title} className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 leading-tight" duration={1.8} />
              <p className="text-lg md:text-xl text-gray-300/90 mb-4 font-medium">{useCase.hero.subtitle}</p>
        ...
        {/* ═══════════════════════════════════════════
            BENEFITS — Numbered cards in 2-column grid
            ═══════════════════════════════════════════ */}
                  ...
                  <div className="group relative h-full">
                    ...
                    <div className="relative h-full bg-gray-900/50 backdrop-blur-sm border border-gray-800/50 rounded-2xl p-6 hover:border-purple-500/30 transition-all duration-300">
                      <div className="flex items-start gap-4">
                        <span className="flex-shrink-0 text-2xl font-mono font-bold text-gray-600 group-hover:text-purple-500/60 transition-colors">{String(i + 1).padStart(2, '0')}</span>
                        <div>
        ...
        {/* ═══════════════════════════════════════════
            RELATED INDUSTRIES — Clickable cards with ArrowRight
            ═══════════════════════════════════════════ */}
              ...
              {useCase.relatedIndustries.map((industry, i) => (
                <AnimatedWrapper key={industry.name} animation="scale-in" delay={i * 0.06}>
                  <Link to={industry.href} className="block h-full">
                    <div className="group relative h-full">
                      <div className="absolute inset-0 bg-gradient-to-br from-purple-500/5 to-blue-500/5 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                      <div className="relative h-full bg-gray-900/50 backdrop-blur-sm border border-gray-800/50 rounded-2xl p-5 hover:border-purple-500/30 transition-all duration-300 text-center">
        ...
        {/* ═══════════════════════════════════════════
            CTA — Bold closing section with gradient text and blur orbs
            ═══════════════════════════════════════════ */}
